...
def recommend_games_multi(input_game_ids, top_k=10, base_bonus=0.2, overlap_bonus_pct=0.05, proximity_bonus=0.05):
...
if __name__ == "__main__":
    ...
    if "train" in sys.argv:
        ...
        example_game_ids = [24869, 4754]  # Resident Evil and Alien: Isolation
        recommendations = recommend_games_multi(example_game_ids, top_k=10)
        print(f"Recommendations for alien and resident evil")
        for gid, score in recommendations:
            gid_int = int(gid)
            filtered = df[df['id'] == gid_int]
            if filtered.empty:
                print(f"Game ID {gid_int} not found in df!")
                continue
            name = filtered['name'].values[0]
            print(f"{name} (ID: {gid}) - Score: {score:.4f}")

        example_game_id = 4754  
        recommendations = recommend_games(example_game_id, top_k=5, bonus_weight=0.3)
        print(f"Recommendations for game ID {example_game_id} ({df[df['id'] == example_game_id]['name'].values[0]})")
        for gid, score in recommendations:
            gid_int = int(gid)
            filtered = df[df['id'] == gid_int]
            name = filtered['name'].values[0] if not filtered.empty else "Unknown"
            print(f"{name} (ID: {gid}) - Score: {score:.4f}")

        example_game_id = 24869  
        recommendations = recommend_games(example_game_id, top_k=5, bonus_weight=0.3)
        print(f"Recommendations for game ID {example_game_id} ({df[df['id'] == example_game_id]['name'].values[0]})")
        for gid, score in recommendations:
            gid_int = int(gid)
